...
-- Drop existing admin-only events policies
DROP POLICY IF EXISTS "Admins can view all events" ON public.events;
DROP POLICY IF EXISTS "Admins can insert events" ON public.events;
DROP POLICY IF EXISTS "Admins can update events" ON public.events;
DROP POLICY IF EXISTS "Admins can delete events" ON public.events;

-- Drop existing admin-only attendance policies
...
CREATE POLICY "Members can view active events attendance" ON public.attendance
  FOR SELECT USING (
    auth.uid() = user_id
    OR EXISTS (
...
CREATE POLICY "Members can manage their own attendance" ON public.attendance
  FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Members can update their own attendance" ON public.attendance
  FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Members can delete their own attendance" ON public.attendance
