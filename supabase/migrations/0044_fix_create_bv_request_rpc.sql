-- Recreate the BV request RPC to allow legacy member-session users and remove auth.uid() dependency
DROP FUNCTION IF EXISTS public.create_bv_request(UUID, TEXT, INTEGER, TEXT);

CREATE FUNCTION public.create_bv_request(
  target_user_id UUID,
