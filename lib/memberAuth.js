...
function applyAdminPreviewToLinks() {
  ...
  try {
    ...
    const previewBtn = document.getElementById('return-to-admin-panel-btn');
    if (previewBtn) {
      previewBtn.style.display = 'inline-flex';
      try {
        const u = new URL(previewBtn.getAttribute('href') || '/admin-dashboard.html', window.location.origin);
        u.searchParams.set('preview', 'admin');
        previewBtn.setAttribute('href', u.pathname + u.search + u.hash);
      } catch {}
    }
  } catch {}
}
