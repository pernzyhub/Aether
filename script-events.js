...
async function loadEvents() {
  ...
  try {
    const { data: events, error } = await supabase
      .from("events")
      .select("*, event_types(name, default_points, color)")
...
async function applyBulkAttendance() {
  ...
  try {
    ...
    const { error: insertError } = await supabase
      .from("attendance")
      .insert(attendanceRecords, { onConflict: "event_id,user_id" });
...
// Event listeners
document.getElementById("event-type-form").addEventListener("submit", createEventType);
