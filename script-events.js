...
function renderRewardLogsList() {
  ...
  listContainer.innerHTML = distributionHistory.map((log, index) => `
    <button type="button" class="reward-log-summary-btn" data-index="${index}" style="text-align:left; width:100%; padding:16px 16px; background:#111; border:1px solid #222; border-radius:10px; color:#fff; cursor:pointer; min-height:130px; display:flex; flex-direction:column; justify-content:space-between; gap:10px;">
      <div style="display:flex; justify-content:space-between; align-items:flex-start; gap:10px;">
        <div>
          <div style="font-weight:bold; font-size:15px; margin-bottom:6px;">${escapeHtml(log.name)}</div>
          <div style="font-size:12px; color:#aaa; margin-bottom:8px;">Saved: ${new Date(log.savedAt || log.created_at).toLocaleString()}</div>
          <div style="font-size:12px; color:#ddd; line-height:1.4; max-width:calc(100% - 100px);">
      ...
      <div style="display:flex; flex-wrap:wrap; gap:8px; margin-top:8px;">
        <span style="font-size:11px; color:#7bf59f; background:#122613; padding:5px 8px; border-radius:999px;">${escapeHtml(log.recipient_count ? `${log.recipient_count} recipients` : `${log.assignments?.length || 0} recipients`)}</span>
        <span style="font-size:11px; color:#7fb8ff; background:#10182b; padding:5px 8px; border-radius:999px;">${escapeHtml(log.total_quantity ? `${log.total_quantity} total quantity` : `${log.assignments?.reduce((sum, item) => sum + (item.quantity || 0), 0)} total quantity`)}</span>
        <span style="font-size:11px; color:#ffb86c; background:#2b1f10; padding:5px 8px; border-radius:999px;">${escapeHtml(log.assignments?.length ? `${log.assignments.length} rows` : '0 rows')}</span>
      </div>
    </button>
  `).join('');
